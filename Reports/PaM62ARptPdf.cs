...
namespace GoWMS.Server.Reports
{
    public class PaM62ARptPdf : PdfPageEvents
    {
        ...
        private void ReportBody(Document document, List<Class6_2_A> ListRpts)
        {
            ...
            {
                if (i == 0) sizes[i] = 1.25f;
                else if (i == 1) sizes[i] = 1.0f;
                else if (i == 2) sizes[i] = 0.75f;
                else if (i == 3) sizes[i] = 0.9f;
                else if (i == 4) sizes[i] = 1.20f;
                else if (i == 5) sizes[i] = 0.7f;
                else if (i == 6) sizes[i] = 0.5f;
                else if (i == 7) sizes[i] = 0.75f;
            ...
            cell = new PdfPCell(new Phrase("PALLET NO", _fontstyeheader))
            {
                HorizontalAlignment = Element.ALIGN_LEFT,
                VerticalAlignment = Element.ALIGN_MIDDLE,
                BackgroundColor = headerBackcolor,
            ...
            foreach (var listRpt in ListRpts)
            {
                ...
                cell = new PdfPCell(new Phrase(listRpt.PalletKey.ToString(), _fontstyebody))
                {
                    HorizontalAlignment = Element.ALIGN_LEFT,
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    BackgroundColor = bodyBackcolor,
